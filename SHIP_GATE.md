...
- [x] `[all]` `verify` script exists (test + build + smoke in one command) (2026-05-12) — `npm run verify` = typecheck + build + test; 792 tests pass
- [x] `[all]` Version in manifest matches git tag (2026-05-12) — v2.4.0 in package.json, tagged on publish; `src/version.ts` drift fixed in v2.3.0 (now reads from `package.json`)
