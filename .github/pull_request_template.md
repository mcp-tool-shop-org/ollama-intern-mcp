...
- [ ] New atom (audit-justified gap — needs tests + CHANGELOG entry; the handbook page is generated by `npm run gen:tool-docs`, not hand-written)
...
- [ ] `npm run gen:tool-docs` if a tool schema, description, or annotation title changed (CI runs `gen:tool-docs:check`)
