...
- **Priorità al cloud con una rete di sicurezza.** Un `RoutingOllamaClient` tenta prima il cloud e, in caso di timeout / 5xx / 429 / problemi di rete, esegue il fallback al profilo locale. Le chiavi non valide (401/403) vengono segnalate in modo evidente tramite un circuito di interruzione persistente anziché degradare silenziosamente per sempre; anche un ID del modello cloud dismesso o con errori di battitura (404) viene segnalato.
- **Non si verifica mai un downgrade silenzioso.** Ogni busta include `backend` (`cloud`|`local`), `degraded` e `degrade_reason`, in modo da sapere sempre quando è stato utilizzato il modello locale anziché quello più grande. Un evento NDJSON `backend_fallback` rende visibile la frequenza del fallback da cloud a locale in `ollama_log_tail`.
- **`ollama_doctor` segnala l'autenticazione e la raggiungibilità del cloud** come un blocco distinto; `ollama-intern-mcp doctor` mostra una sezione "Cloud (primary)".
- Il modello cloud predefinito è `minimax-m3:cloud`; sovrascrivere per livello con `INTERN_CLOUD_MODEL` / `INTERN_CLOUD_DEEP_MODEL` (ad esempio, `deepseek-v3.1:671b`).
...
|---|---|---|
| **Atoms** | 29 | Funzioni di base specifiche per ogni compito. **Originali 15:** `classify`, `extract`, `triage_logs`, `summarize_fast` / `deep`, `draft`, `research`, `corpus_search` / `answer` / `index` / `refresh` / `list`, `embed_search`, `embed`, `chat`. **+13 aggiunte nella versione 2.1.0:** `doctor`, `log_tail`, `batch_proof_check` (operazioni); `code_map`, `code_citation`, `multi_file_refactor_propose`, `refactor_plan` (rifattorizzazione); `artifact_prune`, `hypothesis_drill` (artefatto/breve); `corpus_health`, `corpus_amend`, `corpus_amend_history`, `corpus_rerank` (corpus). **+1 funzione di revisione:** `code_review` (risultati strutturati della revisione del codice, strumento principale; solo per la revisione). Le funzioni che possono elaborare più elementi contemporaneamente (`classify`, `extract`, `triage_logs`) accettano `items: [{id, text}]`. |
...
**Questa è un'opzione che deve essere abilitata esplicitamente ed è disattivata per impostazione predefinita.** Il pacchetto rimane incentrato sull'utilizzo locale con **nessuna trasmissione di dati verso l'esterno**, a meno che non si impostino *entrambi* questi parametri. Chiunque non scelga questa opzione non sarà influenzato.
...
|---|---|---|
| `OLLAMA_CLOUD_PRIMARY` | _(non impostato)_ | **L'interruttore per l'attivazione.** `1`/`true`/`yes`/`on` abilita la priorità al cloud. Non impostato = solo locale, nessuna trasmissione di dati verso l'esterno. |
| `OLLAMA_API_KEY` | _(non impostato)_ | Chiave Bearer per Ollama Cloud. **Obbligatoria** quando il cloud è abilitato (il programma si interrompe all'avvio se mancante). |
| `OLLAMA_CLOUD_HOST` | `https://ollama.com` | Host base del cloud. |
| `INTERN_CLOUD_MODEL` | `minimax-m3:cloud` | Modello cloud per i livelli istantaneo, affidabile e approfondito. |
...
> **La disponibilità dei modelli può variare.** Ollama ritira periodicamente i modelli cloud. `minimax-m3:cloud`, `deepseek-v3.1:671b`, `gpt-oss:120b` e `qwen3-coder:480b` sono le opzioni correnti; controlla [ollama.com/search?c=cloud](https://ollama.com/search?c=cloud) prima di fissare un ID.
