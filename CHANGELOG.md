...
- **The default cloud model was retired upstream.** `INTERN_CLOUD_MODEL` defaulted to `qwen3-coder-next:cloud`, which now returns **HTTP 410 Gone** — so every user who opted into Ollama Cloud on defaults was silently degrading to their local model with `cloud_model_missing`. Repinned to `mistral-large-3:675b-cloud`, chosen by **measurement against this repo's own criterion** rather than from a model card: the default serves instant + workhorse, where short-output tools cap `num_predict` tightly, and a thinking model burns that budget on chain-of-thought and returns an empty reply. On a capped (`num_predict: 40`, `format: json`) classify probe, `mistral-large-3:675b-cloud` returned the correct label while `gpt-oss:120b-cloud`, `minimax-m3:cloud`, `glm-5.3-flash:cloud` **and** `kimi-k2.7-code:cloud` all returned empty — reproducing the 2026-06-09 minimax-m3 incident exactly, and confirming the existing doctrine that big thinking models belong on `INTERN_CLOUD_DEEP_MODEL`. Also note `ollama list` is stale in both directions: it still lists retired ids and omits live ones, so `doctor --cloud-check` is the reliable check.
