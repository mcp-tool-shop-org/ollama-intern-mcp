...
3. **Evidence-backed briefs with server-side citation validation.** Every claim in a brief must cite an evidence id. Unknown ids are stripped server-side before the result returns; weak briefs get `weak: true` rather than smooth confabulation. Language: **"evidence-first. No fiction."** — _qualifier:_ evidence-first means citation-ID-validated; it does not mean every generated claim is semantically grounded in cited content. Operators should treat brief output as a vetted-draft, not authority. Use `weak: true` + coverage_notes + the included excerpts to verify.
