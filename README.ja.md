...
**Claude Code のローカルインターン用ツール。** 42個のツール、証拠に基づいた概要、耐久性のある成果物。
...
`ollama_extract` の各呼び出しに対する、オーバーライド可能なティアごとの予算。 既存の v2.6.0 より前のバージョンを使用しているユーザーへの影響はありません。 詳細については、[CHANGELOG.md](./CHANGELOG.md) を参照してください。

- **`ollama_extract` のスキーマフィールド `tier_budget_ms_override?: number` (オプション、範囲 `[1, 600000]` ms)。** このフィールドが存在する場合、ランナーが訪問するすべてのティアに適用され、`src/guardrails/timeouts.ts:61` にある内部の `runWithTimeoutAndFallback` メカニズムが、プロファイルのデフォルト値ではなく、オペレーターが指定した予算を尊重します。 カスケード処理（ワークホース → タイムアウト時のインスタント）は引き続き機能しますが、オーバーライドは各カスケードの段階で一様に適用されます。
- **この機能の目的。** 研究用オペレーティングシステム (research-os) の R-018 ラッパー (v0.12.1) は、MCP の `callTool` を `Promise.race` でラップし、ラッパーの予算が内部ティアに到達しないことが判明しました。 `DEV_RTX5080_TIMEOUTS.instant = 15_000` が、180000ms のラッパー予算があっても、引き続き 15000ms で `TIER_TIMEOUT` を発生させていました。 v2.6.0 では、MCP 側の権威ある予算を提供することで、オペレーターの `--planner-timeout-ms` フラグ (research-os) が、設計どおりに内部ティアのタイムアウトを制御できるようになりました。
- **デフォルトの動作は維持されます。** このフィールドが省略されている場合、プロファイルのデフォルト値がそのまま適用されます。 v2.6.0 より前のバージョンを使用しているユーザーは、変更はありません。
- **R-010 のフォールバック原因の正規表現は維持されます。** サーバー側の `TIER_TIMEOUT` エラーメッセージは、引き続き `/elapsed=(\d+)ms/` + `/budget=(\d+)ms/` に一致するため、AIアドバイザーによるダウンストリームでの可視化は、オーバーライドとデフォルトの両方のパスで機能します。
- research-os v0.13.0 (累積的な R-019 クライアントの接続処理 + R-020 + R-021) によって、関連する複数のリポジトリでの一括リリースとして提供されます。

### 過去のリリース — v2.4.0 の内容

詳細については、[CHANGELOG.md](./CHANGELOG.md) および [docs/release-notes/v2.4.0.md](./docs/release-notes/v2.4.0.md) を参照してください (プロファイルシステムにおけるティアごとの `num_ctx` の制御)。
...
Claude のすべてのツール呼び出しは、stdio JSON-RPC を介して MCP サーバーに送信されます。 サーバーは、ツールの [zod](https://zod.dev) スキーマに対して呼び出しを検証し、設定されたガードレール（引用の検証、禁止語の削除、保護されたパスの適用、信頼度閾値）を実行し、その後、決定論的なレンダラー（成果物ティア）または Ollama HTTP 呼び出し（他のすべてのティア）のいずれかにルーティングします。 Ollama デーモンは、ユーザーが提供したパスを一切受信しません。受信するのは、モデルティアと準備されたプロンプトのみです。 各呼び出しは、構造化されたイベントを `~/.ollama-intern/log.ndjson` に追加し、`ollama_log_tail` やシェルから読み取ることができます。
...
## この中に含まれるもの — 4つのティア、<!-- TOOL_COUNT:start -->42<!-- TOOL_COUNT:end -->個のツール
...
| レベル | 数 | ここに何があるか |
|---|---|---|
| **Atoms** | 28 | ジョブ指向の基本的な機能。 **最初の15個:** `classify`、`extract`、`triage_logs`、`summarize_fast` / `deep`、`draft`、`research`、`corpus_search` / `answer` / `index` / `refresh` / `list`、`embed_search`、`embed`、`chat`。 **v2.1.0 で追加された13個:** `doctor`、`log_tail`、`batch_proof_check` (運用); `code_map`、`code_citation`、`multi_file_refactor_プロポーズ`、`refactor_plan` (リファクタリング); `artifact_prune`、`hypothesis_drill` (成果物/概要); `corpus_health`、`corpus_amend`、`corpus_amend_history`、`corpus_rerank` (コーパス)。 バッチ処理に対応した機能 (`classify`、`extract`、`triage_logs`) は、`items: [{id, text}]` を受け入れます。 |
...
合計: **28個の機能 + 3個の概要 + 3個のパッケージ + 7個の成果物ツール = <!-- TOOL_COUNT:start -->42<!-- TOOL_COUNT:end -->個。**

凍結対象：
- アトム：**v2.1.0で凍結解除**（現在28個。v2.1.0の機能追加で13個追加）。新しいアトムは、監査による承認、テスト、マニュアルのページ、およびCHANGELOGへの記述が必要であり、安易な追加は認められません。
- パック：3で凍結。新しいパックの種類は追加されません。
- アーティファクトのティア：7で凍結。
...
段階的に機能強化を進めます。パックとアーティファクトのティアは、引き続き3と7で凍結されます。アトムの凍結はv2.1.0で解除されました。新しいアトムは、監査による承認、テスト、マニュアルのページ、およびCHANGELOGへの記述が必要です。
