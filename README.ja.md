...
**Claude Codeのローカルインターンシッププログラム。** 42種類のツール、証拠に基づいたドキュメント、永続的な成果物。
...
Claudeのすべてのツール呼び出しは、stdio JSON-RPCを介してMCPサーバーに送信されます。サーバーは、ツールの[zod](https://zod.dev)スキーマに対して呼び出しを検証し、設定されたガードレール（引用の検証、禁止語の削除、保護されたパスの適用、信頼度閾値）を実行し、その後、決定的なレンダラー（成果物ティア）またはOllama HTTP呼び出し（他のティア）のいずれかにルーティングします。Ollamaデーモンは、ユーザーが提供したパスを一切参照しません。参照するのはモデルティアと、あらかじめ準備されたプロンプトのみです。すべての呼び出しは、`~/.ollama-intern/log.ndjson`にあるNDJSONログに構造化されたイベントを1つずつ追加します。このログは、`ollama_log_tail`やあなたのシェルから読み取ることができます。
...
| レベル | 数 | ここに何があるか |
|---|---|---|
| **Atoms** | 28 | タスク指向の基本的な機能。**オリジナル15個:** `classify`（分類）、`extract`（抽出）、`triage_logs`（ログのトリアージ）、`summarize_fast` / `deep`（高速/詳細な要約）、`draft`（草稿作成）、`research`（調査）、`corpus_search`（コーパス検索）、`answer`（回答）、`index`（インデックス作成）、`refresh`（更新）、`list`（リスト表示）、`embed_search`（埋め込み検索）、`embed`（埋め込み）、`chat`（チャット）。**v2.1.0で追加された13個:** `doctor`（診断）、`log_tail`（ログ追跡）、`batch_proof_check`（バッチ証明チェック）（運用）、`code_map`（コードマップ）、`code_citation`（コード引用）、`multi_file_refactor_propose`（複数ファイルのリファクタリング提案）、`refactor_plan`（リファクタリング計画）（リファクタリング）、`artifact_prune`（成果物の削除）、`hypothesis_drill`（仮説の検証）（成果物/ドキュメント）、`corpus_health`（コーパスの健全性）、`corpus_amend`（コーパスの修正）、`corpus_amend_history`（コーパスの修正履歴）、`corpus_rerank`（コーパスの再ランキング）（コーパス）。バッチ処理に対応した機能（`classify`、`extract`、`triage_logs`）は、`items: [{id, text}]`の形式で複数のアイテムを受け付けます。 |
...
合計：**28個の基本機能 + 3個のドキュメント + 3個のパッケージ + 7個の成果物ツール = 42個**。

固定された機能：
- 基本機能：v2.1.0で**固定が解除**されました（現在28個。v2.1.0で13個追加）。新しい基本機能は、監査による正当化、テスト、ハンドブックのページ、およびCHANGELOGのエントリが必要です。カジュアルな追加は許可されません。
- パッケージ：3個で固定。新しいパッケージの種類は追加されません。
- 成果物ティア：7個で固定。
...
段階的に強化。パッケージと成果物ティアは、それぞれ3個と7個で固定されます。基本機能の固定は、v2.1.0で解除されました。新しい基本機能は、監査による正当化、テスト、ハンドブックのページ、およびCHANGELOGのエントリが必要です。
