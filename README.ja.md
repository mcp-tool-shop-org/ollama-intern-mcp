...
ツールの詳細な参照は、[ハンドブック](https://mcp-tool-shop-org.github.io/ollama-intern-mcp/handbook/tools/) に記載されています。
...
このMCPは、Ollama上の`hermes3:8b`に対して、[Hermes Agent](https://github.com/NousResearch/Hermes)を用いて、エンドツーエンドで検証されています（2026年4月19日）。Hermesは、このMCPの固定された基本的なインターフェースを呼び出す外部エージェントです。Hermesが計画を立て、我々が実行を行います。

設定ファイルの例（このリポジトリ内の[hermes.config.example.yaml](hermes.config.example.yaml)）：
...
**プロンプトの形式が重要です。** 命令形式のツール呼び出しプロンプト（例：「Xを引数…と共に呼び出す」）は、統合テストとして機能します。これにより、8Bのローカルモデルが、適切な構造で`tool_calls`を生成できるようになります。リスト形式のマルチタスクプロンプト（例：「Aを実行し、次にBを実行し、次にCを実行する」）は、より大規模なモデルの性能評価に使用されます。8Bモデルでリスト形式のプロンプトが失敗した場合、それは「接続に問題がある」ことを必ずしも意味しません。詳細な統合テストと、既知の通信に関する注意点（Ollama `/v1`ストリーミングと、openai-SDKの非ストリーミング互換層）については、[handbook/with-hermes](https://mcp-tool-shop-org.github.io/ollama-intern-mcp/handbook/with-hermes/)を参照してください。
...
**デフォルトの開発環境**では、すべてのワークロードを`hermes3:8b`に統合しています。これは、検証済みのHermes Agent連携環境です。同じモデルを使用することで、管理、コスト、理解すべき動作が統一されます。Qwen 3（`THINK_BY_SHAPE`機能を使用）を希望するユーザーは、`dev-rtx5080-qwen3`を選択できます。`m5-max`は、Qwen 3のモデルサイズで、統一メモリ用に最適化されています。
